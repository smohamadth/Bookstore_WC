...
check("vazirmatn-arabic.woff2" in rtl_css and "noto-naskh-arabic.woff2" in rtl_css, "Bundled Persian/Sorani fonts are not loaded")
...
font_licenses = ("OFL-Inter.txt", "OFL-Newsreader.txt", "OFL-Vazirmatn.txt", "OFL-NotoNaskhArabic.txt")
...
check(len(fonts) == 8, f"Expected 8 variable-font files, found {len(fonts)}")
check(len({hashlib.sha256(path.read_bytes()).digest() for path in fonts}) == 8, "Duplicate font binaries remain")
