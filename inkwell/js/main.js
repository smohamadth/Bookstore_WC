...
(function () {
	...
	function initMiniCart() {
		...
		// Keep the dropdown open briefly after "add to cart" so shoppers
		// can see the item land (WC replaces .widget_shopping_cart_content).
	...
	function initQuantitySteppers() {
		document.addEventListener('click', function (e) {
			...
			var current = parseFloat(input.value) || min || 1;
		...
		Array.prototype.forEach.call(forms, function (form) {
			form.addEventListener('submit', function (e) {
				...
				if (!email || !email.value || email.value.indexOf('@') === -1) {
					showNote(note, 'Please enter a valid email address.', 'is-error');
					return;
				}
